...
def remember(user_id: str, content: str, kind: str = "preference") -> None:
    ...
    with SessionLocal() as session:
        session.add(UserMemory(user_id = user_id, content = content, kind = kind))
        session.commit()

def list_active(user_id: str)-> list[UserMemory]:
    """List all active memories for a user."""
    with SessionLocal() as session:
        rows = session.execute(select(UserMemory).where(UserMemory.user_id == user_id, UserMemory.active == True)
...
def deactivate_memory(user_id: str, memory_id: int)->None:
    ...
    with SessionLocal() as session:
        ...
        if rows and rows.user_id == user_id:
            rows.active = False
            session.commit()
